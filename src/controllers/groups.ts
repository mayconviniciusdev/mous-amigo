...
export const addGroup: RequestHandler = async (req, res) => {
  ...
  const newGroup = await groups.add({...body.data, id_event: parseInt(id_event!)})
...
export const updateGroup: RequestHandler = async (req, res) => {
  ...
  const updatedGroup = await groups.update({id: parseInt(id!), id_event: parseInt(id_event!)}, data);
...
export const deleteGroup: RequestHandler = async (req, res) => {
  const {id, id_event} = req.params;
  const deletedGroup = await groups.remove({id: parseInt(id!), id_event: parseInt(id_event!)})
